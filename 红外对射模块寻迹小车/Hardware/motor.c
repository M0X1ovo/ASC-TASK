...
void Motor_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 | GPIO_Pin_11|GPIO_Pin_3 | GPIO_Pin_4| GPIO_Pin_5| GPIO_Pin_13;
...
void Motor_SetPWM(uint8_t motor, int8_t PWM)
{
    if (motor == 1)  // 控制电机左前
    {
        ...
        {
            GPIO_SetBits(GPIOB, GPIO_Pin_4);     // AIN1 = 1
            GPIO_ResetBits(GPIOB, GPIO_Pin_3);   // AIN2 = 0
    ...
    else if (motor == 2)  // 控制电机左后
    {
        ...
        else  // 反转
        {
            ...
            GPIO_ResetBits(GPIOB, GPIO_Pin_5);   // BIN2 = 0
	    ...
	    else if (motor == 4)  // 控制电机右后
    {
        if (PWM >= 0)  // 正转
        {
            GPIO_ResetBits(GPIOA, GPIO_Pin_9);   // BIN1 = 0
            GPIO_SetBits(GPIOA, GPIO_Pin_10);     // BIN2 = 1
